...
gituser="llisandur"
gitrepo="ff1-randomizer-poptracker-pack"
filepre="ff1r"
...
filename="$filepre-ap-tracker.zip"
if [[ -n "$version" ]]; then
  filename="$filepre-ap-tracker-$version.zip"
...
if [[ -n "$version" ]]; then
  ...
  jq --arg ver "$version" \
     --arg url "https://github.com/$gituser/$gitrepo/releases/download/$version/$filepre-ap-tracker-$version.zip" \
     --arg sha "$sha256" \
